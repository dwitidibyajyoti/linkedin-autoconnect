  ...
  chrome.scripting.executeScript(
    ...
    (injectionResult) => {
      ...
      if (injectionResult[0]?.result?.connectList?.length > 0) {
        ...
        document.getElementById("increment_text").textContent =
          connectListStore.length;
...
function getPeopleListInjectScript(test) {
  let peopleList = document.querySelectorAll(
    "ul.reusable-search__entity-result-list button"
  );
...
function sendConnect(btnId) {
  ...
  setTimeout(() => {
    document
      .getElementsByClassName("artdeco-modal__actionbar")[0]
      .querySelector(".artdeco-button--primary")
      .click();
  }, 100);

  //   document.getElementById("artdeco-modal-outlet").addEventListener(
  //     "DOMNodeInserted",
  //     function (event) {
...
function changeUi() {
  let offSetValue =
    233 - Math.round(233 / connectListStore.length) * (connectCount + 1);
